...
var appIndex = new Vue(
		{
			...
			methods : {
                ...
                queryCaseByCondition:function(pageIndex){
                	var vm = this;
                    var url = requestUrl.queryCaseByCondition;
                	var param={};
                ...
                queryProdutionByCondition:function(pageIndex){
                	var vm = this;
                    var url = requestUrl.queryProdutionByCondition;
                	var param = {};
                	param.pageIndex=pageIndex-1;
                	axios.post(url,param).then(
                			function(response){
                ...
                queryLocationByCondition:function(pageIndex){
                    var vm = this;
                    var url = requestUrl.querylocationByCondition;
                	var param = {};
                	param.pageIndex=pageIndex-1;
                	axios.post(url,param).then(
                			function(response){
                ...
                queryLatestOrders: function () {
                    var vm = this, url = requestUrl.trade_status;
                ...
                queryPartners:function(){
                	var vm  = this ;
                    url = requestUrl.webSiteCooperativeLogo;
                	 axios.post(url, {}).then(function (response) {
                         if(Array.isArray(response.data) && response.data.length > 0){
                         	for (var i = 0; i < response.data.length; i++) {
